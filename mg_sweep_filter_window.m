%% ========================================================================
%  SWEEP SCRIPT: FIND OPTIMAL FILTER WINDOW FOR MG FORECASTING
% ========================================================================
clear; close all; clc;

%% --- 1. SETUP THE SWEEP ---
% Define the range of window sizes you want to test. 
% A window of 1 means no filtering (it's the raw average).
...
fprintf('Starting filter window sweep for MG Forecasting...\n');
fprintf('Testing %d different window sizes.\n', numel(window_sizes_to_test));

%% --- 2. LOAD DATA AND RUN DETERMINISTIC SIM (Done once) ---
% Load Smoldyn Data
parent_folder = uigetdir('', 'Select the Main Project Folder (MG_Optimized_Iter74 runs)');
...
params.N = 500; params.k_on = 3.9338e-18; params.k_off = 8.6418; params.T = 1.9288;
params.washout1 = 500; params.num_train_points = 500; params.wheretostarttest = 1200; params.num_test_points = 500;
...
params.predictlength = 6; params.lambda = 1e-6;
params.Nres_base = 50; params.memorywindowlength = 5; params.Nres = params.Nres_base * params.memorywindowlength;
params.TIME_STEP_SMOL = 0.01;

load('MGseries_RK4_tau17_beta0.20_gamma0.1_n10_len5000_dt1.0.mat', 'mackey_glass_series');
series_segment = mackey_glass_series(1 : params.num_tot_points + params.predictlength);
series_normalized = (series_segment - min(series_segment)) / (max(series_segment) - min(series_segment));
input_series  = series_normalized(1:end - params.predictlength);
target_series = series_normalized((params.predictlength+1):end);
...
% Generate plot
figure('Name', 'NRMSE vs. Filter Window (MG Forecasting)');
...
title('Filter Window Optimization for MG Forecasting');
