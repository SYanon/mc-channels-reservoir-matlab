...
for i2 = 1:nP2
    for i1 = 1:nP1
        ...
        if run_config.plot_in_loop
            run_info.run_num = current_run;
            run_info.total_runs = total_runs;
            run_info.p1_name = p1_name;
            run_info.p1_val = run_config.(p1_name);
            run_info.p2_name = p2_name;
            run_info.p2_val = run_config.(p2_name);
            run_info.nrmse_test = nrmse_test;
            
            plot_interim_results_numerical(y_test_hat, narma_data.test_target, run_info);
        end
    end
end
...
function config = get_default_config()
    ...
    % --- ADDED: Plotting flag ---
    config.plot_in_loop = true; % Flag to show plots during the sweep
end
...
function plot_heatmap(X, Y, Z, title_str, config, use_log_z)
    ...
    shading interp;
    cb = colorbar;
    if use_log_z, colormap(jet); ylabel(cb, 'Log10(Value)'); else, colormap(flipud(parula)); ylabel(cb, 'Value'); end
...
% --- ADDED: New plotting function for numerical analysis ---
function plot_interim_results_numerical(y_test_hat, y_test_true, run_info)
    fig_title = sprintf('NARMA Prediction (Run %d/%d): %s=%g, %s=%g', ...
        run_info.run_num, run_info.total_runs, ...
        run_info.p1_name, run_info.p1_val, ...
        run_info.p2_name, run_info.p2_val);
    
    figure('Name', fig_title, 'NumberTitle', 'off');
    
    % Ensure y_test_true has the same length as y_test_hat
    y_test_true_segment = y_test_true(1:length(y_test_hat));

    plot(y_test_true_segment, 'b-o', 'LineWidth', 1.5, 'DisplayName', 'True Target');
    hold on;
    plot(y_test_hat, 'r-', 'LineWidth', 1.5, 'DisplayName', 'Prediction');
    hold off;
    grid on;
    title(sprintf('NRMSE: %.3f', run_info.nrmse_test));
    xlabel('Sample Index');
    ylabel('Value');
