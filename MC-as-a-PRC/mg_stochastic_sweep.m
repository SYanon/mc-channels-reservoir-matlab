% =========================================================================
% improved_mg_stochastic_sweep_v2.m
%
% DESCRIPTION:
%   Final corrected version. This script correctly sweeps both parameters
%   by re-splitting the target data inside the main loop.
% =========================================================================

%% 0. Initialize Environment & Define Configuration
close all;
clear;
fprintf('Starting MODULAR Stochastic Mackey-Glass parameter sweep...\n');

% --- Central Configuration Struct ---
config = get_default_config();

%% 1. Load and Prepare Data (Done ONCE)
% Load the raw time series data once. The input/target split will happen in the loop.
...
for i2 = 1:nP2
    for i1 = 1:nP1
        ...
        % --- DEFINITIVE FIX: Get correct data splits for the CURRENT predictlength ---
        ...
        W_out_num = train_readout(reservoir_states_num, current_mg_data.train_target, run_config);
        [nrmse_train_num, nrmse_test_num] = test_readout(W_out_num, reservoir_states_num, current_mg_data, run_config);
        
        % =================== STOCHASTIC (SMOLDYN) MODEL ===================
        [reservoir_states_smol_raw, success] = load_smoldyn_data(run_config);
        
        if success
            reservoir_states_smol = apply_signal_filter(reservoir_states_smol_raw, run_config);
            W_out_smol = train_readout(reservoir_states_smol, current_mg_data.train_target, run_config);
            [nrmse_train_smol, nrmse_test_smol] = test_readout(W_out_smol, reservoir_states_smol, current_mg_data, run_config);
        else
...
function config = get_default_config()
    ...
    config.movmean_on = false;  %filter application being controlled
    config.N     = 500;
    config.k_on  = 1e-18;
    config.k_off = 1;
    config.N_min = 100;
...
function series_data = load_mg_series(filename, required_len, max_predict_len)
    % --- NEW FUNCTION ---
    % Loads the raw Mackey-Glass series from a file just once.
    try
...
function mg_data = get_data_splits(full_series, run_config)
    % --- NEW FUNCTION ---
    % Creates the correct input/target splits for a given predictlength.
    ...
    % Slice the target series for training and testing sets
...
function [sim, reservoir_states] = run_channel_simulation(input_series, config)
    ...
    t_total = length(input_series) * config.T; % Use length of input_series
    ...
    num_symbols = length(input_series); % Use length of input_series
    reservoir_states = zeros(config.Nres, num_symbols);
    for iSym = 1:num_symbols
        t_symbol_start = (iSym - 1) * config.T;
        start_idx = round(t_symbol_start/config.dt) + 1;
