% =========================================================================
% plot_mg_cubed_numerical_analysis
%
% DESCRIPTION:
%   A modular script to perform a two-parameter sweep for the combined
%   Mackey-Glass CUBED forecasting and transformation task.
% =========================================================================

%% 0. Initialize Environment & Define Configuration
close all;
clear;
fprintf('Starting modular Mackey-Glass CUBED parameter sweep...\n');
...
for i2 = 1:nP2
    for i1 = 1:nP1
        ...
        % --- In-loop plotting block ---
        if run_config.plot_in_loop
            figure('Name', sprintf('MG Cubed Prediction: %s=%g, %s=%g', p1_name, p1_vals(i1), p2_name, p2_vals(i2)));
            plot(mg_data.test_target, 'b-o', 'LineWidth', 1.5, 'DisplayName', 'True Cubed Target');
            hold on;
...
function config = get_default_config()
    ...
    config.D        = 1e-11;
    config.T  = 1;
    % Note: predictlength is now set in the data creation script, not here.
    config.washout1         = 500;
...
function mg_data = load_mg_data(filename, required_len)
    try
        ...
        error('Failed to load MG CUBED data from "%s". Please run createMGcubedseries.m first.', filename);
    end
    
    if length(input_raw) < required_len
        error('Loaded input series is too short. Required length: %d.', required_len);
    end
    
    mg_data.input_series  = input_raw(1:required_len);
    mg_data.target_series = target_raw(1:required_len);
    
    % Split the target series for training and testing
    config = get_default_config();
