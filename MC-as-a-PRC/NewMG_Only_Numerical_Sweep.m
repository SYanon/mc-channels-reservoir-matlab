...
% Mackey-Glass time series params
beta  = 0.2;
tau   = 17;
n     = 10;
gamma = 0.1;
...
%% ========== Nested Loops Over param2 & param1 ==========
...
for i2 = 1:nP2
    ...
    for i1 = 1:nP1
        ...
        if plot_in_loop
            figure; 
            plot(ttnew, mackey_glass_series,'LineWidth',1.5);
            title('Mackey-Glass Time Series'); grid on;
        ...
        %% --- Compute Input Molecule Counts ---
        inp_min = min(input_series);
        inp_max = max(input_series);
        N_i = N_min + (input_series - inp_min)/(inp_max - inp_min) * (N_max - N_min);


        %% --- Time Vector & Pre-allocate ---
        ...
        %% --- Compute Concentration at Receiver ---
        ...
        %% --- Autocorrelation: n_values (full time trace) ---
        ...
        %% --- Autocorrelation: c_values (full time trace) ---
        ...
        %% ============ Training Phase ============
        % We sample the reservoir states at each symbol
        ...
        for iSym = (washout1+1) : (washout1 + num_train_points)
            ...
            colIdx = iSym - washout1;  % 1-based column in sampled_matrix
        ...
        % If we have zero columns left, skip
        ...
        % Add bias row
        nCols = size(sampled_matrix, 2);  % # of valid training points
        sampled_matrix = [sampled_matrix; ones(1, nCols)];

        % Ensure y_train is at least as long as nCols
        if nCols > length(y_train)
            warning('Not enough y_train data for %s=%g, %s=%g. Skipping.', ...
                     param1_name, val1, param2_name, val2);
            continue;
        end
        y_tilda = y_train(1:nCols);
        y_tilda = y_tilda(:);

        % Transpose so it is (#trainingSamples x #features)
        sampled_matrix = sampled_matrix';

        % (#features x #features) -> ridge regression (using pinv now)
        ...
        rmse_train = sqrt(mean((y_train_hat - y_tilda).^2));
        nrmse_train = rmse_train / std(y_tilda);
        ...
        if nColsTest > length(y_test)
            warning('Not enough y_test data for %s=%g, %s=%g. Skipping.', ...
                     param1_name, val1, param2_name, val2);
            continue;
        end
        y_test_segment = y_test(1:nColsTest);
        y_test_segment = y_test_segment(:);

        sampled_matrix_test = sampled_matrix_test'; % (#testSamples x #features)
        y_test_hat = sampled_matrix_test * W_out;

        rmse_test = sqrt(mean((y_test_hat - y_test_segment).^2));
        nrmse_test = rmse_test / std(y_test_segment);

        %% ----------- Compute Timescale from ALL Reservoir Samples -----------
        sampled_matrix_all = [];  % size [Nres, #validSymbols]
        ...
        tau_corr_rc_samples = lags_rc(idx_eRC)*dtsample;  % in seconds
        ...
        fprintf('%s=%g, %s=%g -> NRMSE(Tr)=%.3g, NRMSE(Te)=%.3g, Occ=%.3g, tauChar=%.3g, RCsamplesTau=%.3g\n',...
                 param1_name, val1, param2_name, val2, ...
...
maxLagMG = 200;
mg_centered = mackey_glass_series - mean(mackey_glass_series);
...
tauMG_1e_time = tauMG_1e * T * dtMG;

figure('Name','MG Autocorrelation','Position',[300 100 600 400]);
...
fprintf('MG correlation time ~ %d samples ~ %g seconds.\n',...
        tauMG_1e, tauMG_1e_time);
...
Zvals = tscale_char_array';  % (nP2 x nP1)
...
% disp('All done! Code ran successfully, including correlation time of the sampled reservoir states.');
...
% % 4) Timescale Characteristic (theory)
...
% % 6) Timescale from data (c-values)
...
% %% ---- NEW 1D Plot: Timescale from Reservoir Samples ----
