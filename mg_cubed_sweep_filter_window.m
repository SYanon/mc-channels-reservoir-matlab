%% ========================================================================
%  SWEEP SCRIPT: FIND OPTIMAL FILTER WINDOW FOR MG CUBED
% ========================================================================
clear; close all; clc;
...
fprintf('Starting filter window sweep for MG Cubed...\n');
fprintf('Testing %d different window sizes.\n', numel(window_sizes_to_test));

%% --- 2. LOAD DATA AND RUN DETERMINISTIC SIM (Done once) ---
% Load Smoldyn Data
parent_folder = uigetdir('', 'Select the Main Project Folder (MG_Cubed_Smoldyn_Iter91 runs)');
...
params.N = 500; params.k_on = 8.0858e-18; params.k_off = 3.5861; params.T = 1.9251;
...
load('MGCubed_series_k10.mat', 'input_series', 'target_series');
input_series = input_series(1:params.num_tot_points);
target_series = target_series(1:params.num_tot_points);
...
% Generate plot
figure('Name', 'NRMSE vs. Filter Window (MG Cubed)');
...
title('Filter Window Optimization for MG Cubed');
